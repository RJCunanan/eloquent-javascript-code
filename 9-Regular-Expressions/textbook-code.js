...
// Regexp are objects that represent patterns in strings. They use their
// own language to express these patterns.
...
// A regex has a method test() to test whether a given string matches
// it. It also has method exec() that, when a match is found, returns
// an array containing all matched groups. Such an array has an "index"
// property that indicates where the match started.

// Strings have a match() method to match them agains a regex and a
// search() method to search for one, returning only the starting
// position of the match. Their replace() method can replace matches
// of a pattern with a replacement string or function.

// Regex can have options, written after the closing slash. The "i" option
// makes the match case insensitive. The "g" option makes the expression
// global, which, among other things, causes the replace() method to
// replace all instances instead of just the first. The "y" option makes an
// expression "sticky", which means that it will not search ahead and skip
// part of the string when looking for a match. The "u" option turns on
// Unicode mode, which enables \p syntax and fixes a number of problems around
// the handling of characters that take up two code units.

// Regex are a shart tool with an awkward handle. They simplify some tasks
// tremendously but can quickly become unmanageable when applied to complex
// problems. Part of knowing how to use them is resisting the urge to try
// to shoehorn things into them that they cannot cleanly express.
